//-----------------------------------------------------------------------
// <copyright file="ServerJsonReader.cs">
// </copyright>
// <author>liul</author>
...
namespace LL.Framework.Tcl.Parser
{
    ...
    /// Server JSON响应通用读取器.
    /// </summary>
    public class ServerJsonReader : IServerReader
    {
        #region 全局私有变量
        /// <summary>
        /// JSON数据字典.
        ...
        #region 外部服务方法
        /// <summary>
        /// Server JSON响应通用读取器构造函数.
        /// </summary>
        /// <param name="json">JSON数据字典.</param>
        public ServerJsonReader(IDictionary json)
        {
            this.json = json;
        }
                ...
                if (jsonList != null && jsonList.Count > 0)
                {
                    ...
                    foreach (object item in jsonList)
                    {
                        if (typeof(IDictionary).IsAssignableFrom(item.GetType())) // object
                        {
                            ...
                            object subObj = convert(new ServerJsonReader(subMap), type);
                            if (subObj != null)
                            {
                                listObjs.Add(subObj);
                            }
